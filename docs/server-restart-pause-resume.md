...
| **3. Drain endpoint + matchmaking guard + countdown** | Server-side shipped: `POST /admin/drain` (token-gated, idempotent, rate-limited 10/min/IP), `POST /admin/drain/cancel`, separate `MISTBOARD_DRAIN_TOKEN` env var, matchmaking guards on `/api/rooms` and `/api/lobby` (both return 503 + `restartAt`), WS broadcast `server_restart_scheduled`/`server_restart_cancelled` to all connected clients. Configurable: `MISTBOARD_DRAIN_WINDOW_DEFAULT_MS` (15min), `MISTBOARD_DRAIN_WINDOW_MAX_MS` (1h). Client banner UI deferred until users appear. | Server shipped; client banner deferred |
...
| **6. Stale-sweep job** | Periodic finalize of paused rooms older than the window. | Planned |
| **7. `safe-deploy.sh`** | Script: drain → poll for zero-active → trigger Railway deploy. Manual gate first; CI later. | Planned |
...
**Phase 6 stale-sweep hardening (required before that phase ships):**

- Wrap the finalize INSERT in a `WHERE NOT EXISTS (terminal event for this room)` precondition so a late reconnect-triggered resume does not race with the sweep into producing two terminal events. This mirrors the termination-check guard added after the 2026-05-14 resign incident.
