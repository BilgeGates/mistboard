...
// Generated data modules (mined puzzle corpora, generated sample games and
// diagrams): large by construction, regenerated by tooling, never split
// candidates. Ranked in their own section so the code ranking stays useful.
const GENERATED_DATA_PATTERNS = [
  /^packages\/game\/src\/puzzles-[a-z0-9-]+-(?:source-games|tactics|mined|material)\.ts$/,
  /^apps\/web\/src\/shogi4-(?:sample-game|rules-diagrams)\.ts$/,
];

function isGeneratedData(repoPath) {
  return GENERATED_DATA_PATTERNS.some((pattern) => pattern.test(repoPath));
}
...
function printLargeFiles(stats) {
  const codeStats = stats.filter(
    (item) => !CONTENT_HEAVY_FILES.has(item.path) && !isGeneratedData(item.path),
  );
  const generatedStats = stats.filter((item) => isGeneratedData(item.path));
  ...
  if (generatedStats.length > 0) {
    console.log('\n## Generated data modules (not split candidates)');
    for (const item of generatedStats) {
