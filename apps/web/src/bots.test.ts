...
describe('bot pages', () => {
  ...
  it('groups the bot directory by featured and community bots', async () => {
    ...
    expect(root.querySelector('.bot-rating-strip')?.textContent).toContain('Half Xiangqi Rapid');
  ...
  it('renders the bot profile rating and supported variants', async () => {
    ...
    expect(root.querySelector('.bot-profile-variants')?.textContent).toContain('Half Xiangqi');
  });
