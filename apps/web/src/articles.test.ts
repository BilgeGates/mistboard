...
describe('article public listing gates', () => {
  ...
  it('keeps the base Shogi rules article out of the rules rail while leaving it reachable', () => {
    ...
    expect(darkShogi.querySelector('.article-variant-sidebar a[href="/rules/dark-shogi"]'))
      .not.toBeNull();
...
describe('rules variant sidebar', () => {
  ...
  it('lists the rail as one flat variant list in canonical order', () => {
    ...
    expect(hrefs.indexOf('/rules/dark-chess')).toBeLessThan(
      hrefs.indexOf('/rules/dark-shogi'),
    );
  });
