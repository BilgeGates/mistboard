...
function boardsPanel(postgame: JieqiPostgameResponse, signal: AbortSignal): HTMLElement {
  ...
  const boardTargets: Array<{
    ...
    entry: { key: JieqiPostgameViewKey; label: string; view: JieqiPlayerView };
  }> = [];
  ...
  const syncReplay = () => {
    for (const { board, capturesTop, capturesBottom, entry } of boardTargets) {
      ...
      // Jieqi has NO fog: the truth view renders every identity revealed; the
      // per-color views render the opponent's face-down pieces as backs (the
  ...
  for (const entry of views) {
    ...
    board.setAttribute('aria-label', `${entry.label} final Jieqi board`);
