...
import {
  type CrossroadsChessPostgamePersistence,
  crossroadsChessPostgameForApi,
} from './crossroads-chess.js';

const ROOM_ID = 'dchess_postgame';

function finishedResignationEvents(): CrossroadsChessEvent[] {
  return [
    {
      type: 'room-created',
      at: 1,
...
function deps(
  ...
  events: CrossroadsChessEvent[] | null,
): CrossroadsChessPostgamePersistence {
  return {
...
test('Crossroads postgame returns full-board views and per-ply history', async () => {
  const payload = await crossroadsChessPostgameForApi(
    ROOM_ID,
    deps(gameRecord(), finishedResignationEvents()),
  );
  assert.ok(payload);

  assert.equal(payload.game.variant, CROSSROADS_CHESS_SPEC_ID);
...
test('Crossroads postgame rejects unfinished or non-Crossroads games', async () => {
  ...
  assert.equal(await crossroadsChessPostgameForApi(ROOM_ID, deps(gameRecord(), unfinished)), null);
  assert.equal(
    await crossroadsChessPostgameForApi(
      ROOM_ID,
  ...
  assert.equal(
    await crossroadsChessPostgameForApi(ROOM_ID, deps(null, finishedResignationEvents())),
    null,
  );
  assert.equal(await crossroadsChessPostgameForApi(ROOM_ID, deps(gameRecord(), null)), null);
});
