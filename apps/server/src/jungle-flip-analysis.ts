...
async function poolMeanWin(
  ...
  const wins = await Promise.all(
    entries.map((entry) => {
      // Counterfactual: this face-down square is (entry.color, entry.role) instead.
      // applyJungleFlipMove reveals it on the flip (and binds first-mover ink at ply 0).
...
export const JUNGLE_FLIP_DECISIONS_ENGINE_ID = `misty-jungle-flip-decisions@${JUNGLE_FLIP_ENGINE_VERSION}`;
