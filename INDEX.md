...
| `landing.ts` | Mounts for landing / game / contact. Owns homepage replay wiring, public game review shell, and contact route handoff. Shell, game-display, shared game metadata, homepage showcase data, announcement panel helpers, play/setup/lobby flows, and watch route ownership have moved out, so route modules no longer import from landing. Loads `landing.css` for homepage/game route CSS currently owned by this module. |
| `landing.css` | Homepage and public game-review route styles loaded by `landing.ts` |
