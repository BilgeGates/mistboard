...
| `replay.ts` | Replay viewer: `mountReplay` closure, panels, annotation form. Wall-clock loop timing helpers live in `replay-wall-clock.ts`; replay move-list panel UI lives in `replay-moves-panel.ts`; game metadata/header UI lives in `replay-meta.ts`; replay clock panel rendering lives in `replay-clocks.ts`; board/pane adapters live in `replay-board.ts`. |
