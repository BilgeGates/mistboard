...
| `landing.ts` | Mounts for landing / watch / game / contact. Lobby + create-room flows, recent-games render, landing widgets, setup dialog. Shell, game-display, homepage showcase data, and announcement panel helpers have moved out, so route modules no longer import from landing. Loads `landing.css` for homepage/watch/game route CSS currently owned by this module. |
