...
| `landing.ts` | Mounts for landing / game / contact. Owns homepage replay wiring, public game review shell, and contact route handoff. Shell, game-display, shared game metadata, homepage showcase data, announcement panel helpers, play/setup/lobby flows, and watch route ownership have moved out, so route modules no longer import from landing. Loads `landing.css` and `game-route.css` |
| `landing.css` | Homepage route styles loaded by `landing.ts` |
