...
| `landing.ts` | Mounts for landing / watch / game / contact. Lobby + create-room flows, recent-games render, landing widgets, setup dialog. Shell, game-display, homepage showcase data, and announcement panel helpers have moved out, so route modules no longer import from landing. Loads `landing.css` for watch/game route CSS currently owned by this module. |
| `landing.css` | Watch route and public game-review route styles loaded by `landing.ts` |
| `landing-announcements.ts` | Landing announcement panel/card renderer: announcement ordering, article thumbnail lookup, kind/date labels, and CTA labels |
