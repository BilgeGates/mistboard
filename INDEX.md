...
| `landing.ts` | Mounts for landing / watch / game / contact. Owns homepage replay wiring, watch replay feed, public game review shell, and contact route handoff. Shell, game-display, homepage showcase data, announcement panel helpers, and play/setup/lobby flows have moved out, so route modules no longer import from landing. Loads `landing.css` for homepage/watch/game route CSS currently owned by this module. |
| `landing.css` | Homepage, watch route, and public game-review route styles loaded by `landing.ts` |
