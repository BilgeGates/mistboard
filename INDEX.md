...
| `server-xiangqi-engine.ts` | Server-side mainline-Pikafish PvE loop for Standard Xiangqi: `scheduleXiangqiEngineMove`, `playXiangqiEngineMoveIfReady`, seat/UCI helpers; injects engine moves through the same append+broadcast path as humans (board moves only, Pikafish native rank-0-9 coords) |
...
| `xiangqi-analysis.ts` | Whole-game postgame analysis orchestration (`analyzeXiangqiGame`, `PlyEval`): evaluates every ply Red-POV via Pikafish through the shared engine pool, sequentially, for the advantage chart and move judgments |
...
| `xiangqi-fsf-engine.ts` | Fairy-Stockfish provider for deliberately weakened human-facing Standard Xiangqi opponents (`fairy-stockfish-xiangqi-level-1..8`, Lichess-style skill/depth/movetime tiers): `xiangqiFsfLiveEngineMove` plus FSF-to-Pikafish UCI coord conversion |
...
| `engine-time-budget.ts` | `budgetForMove`: shared clock-aware per-move time allocator (moves-to-go + increment) for the UCI-subprocess engine fleet (banqi/crossroads/drop-mini/fortress/jieqi/jungle/mini-xiangqi); returns a movetime CEILING, the node budget stays the strength anchor |
...
| `luzhanqi-tenant.ts` | Luzhanqi `VariantTenant` (computer-refereed two-player dark Junqi, `lzq_` room prefix, `luzhanqiEnabled` flag): setup-formation event model (each seat locks a private formation before play starts), server-truth battle identities, per-seat view revealing only own ranks plus explicit Marshal-loss flags |
| `luzhanqi-runtime.ts` | Luzhanqi live-room type aliases over the generic `variant-tenant/` runtime (types-only; ws/factory/routes/registration call the generic `tenant*` functions directly) |
| `luzhanqi-registration.ts` | Luzhanqi registry entry (`luzhanqiRooms` map, `createLuzhanqiRoom`, `getOrLoadLuzhanqiRoom`): room-factory binding + hydration; flag-gated hidden/dev-only |
| `server-luzhanqi-room-factory.ts` | Thin adapter over `variant-tenant/room-factory.ts` for Luzhanqi (`createLuzhanqiLiveRoom`; PvP-only casual rooms, `luzhanqi_disabled`/`room_id_collision`/`persistence_failure` results) |
| `server-ws-luzhanqi.ts` | Thin adapter over `variant-tenant/ws.ts` for Luzhanqi (`handleLuzhanqiWebSocketConnection`, lifecycle timer bind/clear); own-ranks-only per-seat redaction rides the tenant hooks |
| `routes/luzhanqi-rooms.ts` | Luzhanqi room-creation branch for `POST /api/rooms` (`requestsLuzhanqi`, `handleLuzhanqiCreate`): factory-bound (`variant-tenant/rooms-route.ts`), PvP-only, no engine, rejects rated as surface, disabled-flag gate |
...
| `persistence-studies.ts` | User-created study persistence (092): study + ordered chapters (serialized JSONB move tree), optimistic-concurrency `version` guard, owner CRUD, visibility-gated public lists, like state; no-ops when persistence is disabled |
| `routes/studies.ts` | Study CRUD API (092): `POST /api/studies`, `/mine`, `/:id` GET/PATCH/DELETE, `/:id/chapters` + `/:cid` (version-guarded save/rename/delete), `/like`; owner-only writes, visibility-gated reads, fail-closed variant allowlist (xiangqi only) |
...
| `review/review-layout.ts` | `createReviewScaffold` (pure layout: hugging shell, review-stage, underboard, eval gauge, rail material, board zoom) plus `mountReviewLayout` (linear ply-scrubber controller) plus `createReviewNavBar`/`installReviewKeyboard`; the shared layout every /game review rides, navigation-agnostic |
...
| `review/captured-diff.ts` | `capturedByDiff`: compute captured material by diffing a variant's initial board against the current one, for postgame surfaces whose player-view carries no captured list (jungle, mini-xiangqi, dark-xiangqi); entry `owner` is the color that LOST the piece |
| `review/captured-pool.ts` | `fillCapturedPoolWith`: generic captured-material pool for review surfaces with non-xiangqi renderers (jungle animal tokens, dark xiangqi glyphs); caller supplies the single-piece renderer, role repeats collapse to one glyph plus a count badge |
...
| `review/engine/engine-arrows.ts` | `engineArrowsFromLines`/`bestMoveArrow`: pure engine-output-to-board-arrow mapping for the review board overlay; rank encoded as visual weight (PV1 heaviest, drawn last) |
...
| `review/jungle-review.ts` | `mountJungleReview`: Jungle (Dou Shou Qi) review presentation bundle over mountTreeReview; perfect-info, drives the in-browser MistyJungle wasm engine with the full-board FEN (no redaction), the first non-xiangqi consumer of mountTreeReview |
...
| `review/fortress-xiangqi-review.ts` | `mountFortressXiangqiReview`: Fortress Xiangqi review presentation bundle over mountTreeReview; perfect-info, live eval gauge/engine panel/Share FEN via client ceval (FSF `fortressxiangqi`), on-board PV arrows and drop gestures deferred |
...
| `videos-data.ts` | Curated /videos library data: `VIDEO_TAGS`, the `VideoEntry` shape, and the `VIDEOS` list (titles/authors verbatim from YouTube oembed); consumed by videos.ts |
| `videos.ts` | The `/videos` page (lichess video style): a filterable grid of link-out YouTube cards; exports `mountVideos`, `buildVideosPage`, and `filterVideos`, fed by videos-data.ts |
...
| `historical-xiangqi-search.ts` | Historical xiangqi game search page: filterable list over the search API (mistboard / historical / broadcast sources) linking to per-game review URLs. Exports `mountHistoricalXiangqiSearch`, result/outcome + URL helpers, and the list/response types. Loads `historical-xiangqi-search.css` |
| `study-index.ts` | `/study` browse page: All studies (`/api/studies/public`) and My studies (`?tab=mine`, `/api/studies/mine`) tabs, plus a create-study metadata dialog that opens the fresh study. Exports `mountStudyIndex` |
| `study.ts` | `/study/:id` study viewer/editor: rebuilds each chapter's serialized tree and mounts the shared xiangqi review (chapters as tabs); owner edits autosave via the version-guarded chapter PATCH and can add/delete chapters, non-owners get read/explore. Exports `mountStudy` |
...
| `verify-title.ts` | `/verify-title` page: signed-in players request a title from the closed vocabulary with free-text evidence, submitting for admin review at `/titles`; one pending request at a time, signed-out get a sign-in prompt. Exports `mountVerifyTitle`. Loads `verify-title.css` |
...
| `jungle-board.ts` | Interactive Jungle (Dou Shou Qi) board for review/analysis: factory `createJungleInteractiveBoard` over the same shared installBoardDrag + installSelectionClickAway as the live board. No engine or overlay, so setArrows/setMarkers are no-ops |
| `fortress-xiangqi-board.ts` | Interactive Fortress Xiangqi board for review/analysis: factory `createFortressXiangqiInteractiveBoard` over the shared installBoardDrag + installSelectionClickAway. Board moves only (hand drops not yet a gesture); setArrows/setMarkers are no-ops |
| `live-luzhanqi.ts` | Live multiplayer room client for Luzhanqi (Junqi / Land Battle), a hidden-information tenant on the generic live-client core, gated by `luzhanqiEnabled`. Exports `bootstrapLuzhanqiLiveRoom`. Loads `luzhanqi-preview.css` |
| `luzhanqi-preview.ts` | DEV `/luzhanqi-preview` page: renders the Luzhanqi board (camps, HQs, mountains, frontlines) and a seeded random playout across red / black / truth views. Exports `mountLuzhanqiPreview`. Loads `luzhanqi-preview.css` |
| `luzhanqi-skin.ts` | Luzhanqi piece-skin vocabulary: `ROLE_SKIN` maps each rank to an animal/builder/den/stone/trap token, plus `roleDisplayName` and the `renderLuzhanqiSkinMark` SVG builder |
...
| `dobutsu-ui-icons.ts` | Dobutsu UI icon set: `buildDobutsuUiIcon` (`/ui/dobutsu/*.png`) and `dobutsuIconForAnnouncementKind`. Loads `dobutsu-ui-icons.css` |
| `dobutsu-ui-preview.ts` | DEV `/dobutsu-ui-preview` gallery for the pixel-lab Dobutsu UI icon assets, shown against landing/forum styles. Exports `mountDobutsuUiPreview` |
...
Numbered raw SQL files starting at `001_init.sql`; the count moves fast (103+ as of 2026-07-16), so check `ls apps/server/migrations | tail` for the latest. Three legacy duplicate numeric prefixes exist (051/064/081); new migrations get unique prefixes. Runner: `migrate.ts` (Postgres advisory-lock + `_migrations` table). Add schema changes as new numbered migrations only.
