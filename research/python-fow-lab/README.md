...
This is **not** part of the Bichess product. The Bichess TypeScript product owns the canonical Fog of War game (server-authoritative, in `apps/server` + `packages/game`). This lab exists for offline work that is easier in Python:
...
Origin: consolidated from `~/projects/fog-of-war-chess/` on 2026-05-05. The standalone Python lab repo was retired because Bichess now hosts both the playable product and the research surface.
...
1. described in the relevant doc under `docs/fog-of-war/` at the bichess root, and
