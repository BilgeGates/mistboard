...
_FEEDBACK = Path(__file__).parents[1] / "feedback"
...
def test_apply_move_real_game_replay():
    """Replay all 84 games; at every position try EVERY pseudo-legal
    move and compare resulting FEN byte-for-byte against python-chess."""
    if not _FEEDBACK.exists():
        pytest.skip("no feedback/ dir — needs real game corpus")
    paths = sorted(_FEEDBACK.glob("mirror-*/games/*.jsonl"))
    if not paths:
        pytest.skip("no real game files found")
    errors: list[str] = []
    ...
    for game_path in paths:
        ...
        for mv in moves:
            ...
            board.push(mv)
            _check_all_pseudo_legal(board, errors, counters)
    assert counters["total"] >= 50_000, (
        f"expected ≥50K (prev, move) tuples tested, got {counters['total']}"
    )
    if counters["diffs"]:
