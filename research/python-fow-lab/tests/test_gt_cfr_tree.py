...
def test_puct_higher_q_wins_when_all_actions_visited_equally():
    ...
    for _ in range(10):
        state.visit_counts[info_set][a_good] += 1
        state.value_sum[info_set][a_good] += 0.5
        state.value_sq_sum[info_set][a_good] += 0.25
        state.visit_counts[info_set][a_bad] += 1
        state.value_sum[info_set][a_bad] += -0.5
        state.value_sq_sum[info_set][a_bad] += 0.25
...
def test_puct_under_visited_action_wins_when_q_tied():
    ...
    for _ in range(50):
        state.visit_counts[info_set][a_seen] += 1
        state.value_sum[info_set][a_seen] += 0.0
        state.value_sq_sum[info_set][a_seen] += 0.5  # nonzero variance
    state.visit_counts[info_set][a_fresh] += 1
    state.value_sum[info_set][a_fresh] += 0.0
    state.value_sq_sum[info_set][a_fresh] += 0.5
