...
def move_from_protocol(m: proto.Move) -> chess.Move:
    promo = _PIECE_LETTER_TO_TYPE[m.promotion] if m.promotion else None
    return chess.Move(m.from_square, m.to_square, promotion=promo)


def move_to_protocol(m: chess.Move) -> proto.Move:
    promo: Optional[proto.PieceLetter] = (
        _PIECE_TYPE_TO_LETTER[m.promotion] if m.promotion else None
    )
